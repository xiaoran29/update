...
# ========= 频道分类存储变量定义 =========
# 保持v0.00版本的分类结构，但使用v0.01的变量名
yangshi_lines = []      # 央视频道 (对应v0.00的ys_lines)
weishi_lines = []       # 卫视频道 (对应v0.00的ws_lines)

beijing_lines = []      # 北京 (对应v0.00的bj_lines)
shanghai_lines = []     # 上海 (对应v0.00的sh_lines)
guangdong_lines = []    # 广东 (对应v0.00的gd_lines)
jiangsu_lines = []      # 江苏 (对应v0.00的jsu_lines)
zhejiang_lines = []     # 浙江 (对应v0.00的zj_lines)
shandong_lines = []     # 山东 (对应v0.00的shandong_lines)
sichuan_lines = []      # 四川 (对应v0.00的sc_lines)
henan_lines = []        # 河南 (对应v0.00的hen_lines)
hunan_lines = []        # 湖南 (对应v0.00的hn_lines)
chongqing_lines = []    # 重庆 (对应v0.00的cq_lines)
tianjin_lines = []      # 天津 (对应v0.00的tj_lines)
hubei_lines = []        # 湖北 (对应v0.00的hb_lines)
anhui_lines = []        # 安徽 (对应v0.00的ah_lines)
fujian_lines = []       # 福建 (对应v0.00的fj_lines)
liaoning_lines = []     # 辽宁 (对应v0.00的ln_lines)
shaanxi_lines = []      # 陕西 (对应v0.00的sx_lines)
hebei_lines = []        # 河北 (对应v0.00的heb_lines)
jiangxi_lines = []      # 江西 (对应v0.00的jx_lines)
guangxi_lines = []      # 广西 (对应v0.00的gx_lines)
yunnan_lines = []       # 云南 (对应v0.00的yunnan_lines)
shanxi_lines = []       # 山西 (对应v0.00的shanxi_lines)
heilongjiang_lines = [] # 黑龙江 (对应v0.00的hlj_lines)
jilin_lines = []        # 吉林 (对应v0.00的jl_lines)
guizhou_lines = []      # 贵州 (对应v0.00的gz_lines)
gansu_lines = []        # 甘肃 (对应v0.00的gs_lines)
neimenggu_lines = []    # 内蒙古 (对应v0.00的nm_lines)
xinjiang_lines = []     # 新疆 (对应v0.00的xj_lines)
hainan_lines = []       # 海南 (对应v0.00的hain_lines)
ningxia_lines = []      # 宁夏 (对应v0.00的nx_lines)
qinghai_lines = []      # 青海 (对应v0.00的qh_lines)
xizang_lines = []       # 西藏 (v0.00无对应，但保留)

hongkong_lines = []    # 香港 (对应v0.00的gat_lines部分)
macau_lines = []       # 澳门 (对应v0.00的gat_lines部分)
taiwan_lines = []      # 台湾 (对应v0.00的gat_lines部分)

digital_lines = []     # 数字 (对应v0.00的auto_4k_lines)
movie_lines = []       # 电影 (对应v0.00的dy_lines)
tv_drama_lines = []    # 电视剧 (对应v0.00的dsj_lines)
documentary_lines = [] # 纪录片 (对应v0.00的jlp_lines)
cartoon_lines = []     # 动画片 (对应v0.00的dhp_lines)
radio_lines = []       # 收音机 (对应v0.00的radio_lines)
variety_lines = []     # 综艺 (对应v0.00的zy_lines)
huya_lines = []        # 虎牙 (v0.00无对应，但保留)
douyu_lines = []       # 斗鱼 (v0.00无对应，但保留)
commentary_lines = []  # 解说 (对应v0.00的js_lines)
music_lines = []       # 音乐 (对应v0.00的yy_lines)
food_lines = []        # 美食 (v0.00无对应，但保留)
travel_lines = []      # 旅游 (v0.00无对应，但保留)
health_lines = []      # 健康 (v0.00无对应，但保留)
finance_lines = []     # 财经 (v0.00无对应，但保留)
shopping_lines = []    # 购物 (v0.00无对应，但保留)
game_lines = []        # 游戏 (对应v0.00的game_lines)
news_lines = []        # 新闻 (v0.00无对应，但保留)
china_lines = []       # 中国 (v0.00无对应，但保留)
international_lines = [] # 国际 (对应v0.00的gj_lines)
sports_lines = []      # 体育 (对应v0.00的ty_lines)
tyss_lines = []        # 体育赛事 (对应v0.00的tyss_lines)
mgss_lines = []        # 咪咕赛事 (对应v0.00的mgss_lines)
traditional_opera_lines = [] # 戏曲频道 (对应v0.00的xq_lines)
spring_festival_gala_lines = [] # 历届春晚 (对应v0.00的cw_lines)
camera_lines = []      # 景区直播（直播中国）(对应v0.00的zb_lines)
favorite_lines = []    # 收藏频道 (v0.00无对应，但保留)
...
# 处理单行频道信息的函数（保持v0.00的分发逻辑，但使用v0.01的变量名）
def process_channel_line(line):
    if "#genre#" not in line and "#EXTINF:" not in line and "," in line and "://" in line:
        ...
        if channel_address not in combined_blacklist:
            # 根据行内容判断存入哪个对象，开始分发（保持v0.00的判断顺序）
            if "CCTV" in channel_name and check_url_existence(yangshi_lines, channel_address) : #央视频道
                yangshi_lines.append(process_name_string(line.strip()))
            elif channel_name in weishi_dictionary and check_url_existence(weishi_lines, channel_address): #卫视频道
                weishi_lines.append(process_name_string(line.strip()))
            elif channel_name in sports_dictionary and check_url_existence(sports_lines, channel_address):  #体育频道
                sports_lines.append(process_name_string(line.strip()))
            elif any(tyss_keyword in channel_name for tyss_keyword in tyss_dictionary) and check_url_existence(tyss_lines, channel_address):  #体育赛事
                tyss_lines.append(process_name_string(line.strip()))
            elif any(mgss_keyword in channel_name for mgss_keyword in mgss_dictionary) and check_url_existence(mgss_lines, channel_address):  #咪咕赛事
                mgss_lines.append(process_name_string(line.strip()))
            elif channel_name in movie_dictionary and check_url_existence(movie_lines, channel_address):  #电影频道
                movie_lines.append(process_name_string(line.strip()))
            elif channel_name in tv_drama_dictionary and check_url_existence(tv_drama_lines, channel_address):  #电视剧频道
                tv_drama_lines.append(process_name_string(line.strip()))
            elif channel_name in shanghai_dictionary and check_url_existence(shanghai_lines, channel_address):  #上海频道
                shanghai_lines.append(process_name_string(line.strip()))
            elif channel_name in hongkong_dictionary and check_url_existence(hongkong_lines, channel_address):  #港澳台（香港）
                hongkong_lines.append(process_name_string(line.strip()))
            elif channel_name in macau_dictionary and check_url_existence(macau_lines, channel_address):  #港澳台（澳门）
                macau_lines.append(process_name_string(line.strip()))
            elif channel_name in taiwan_dictionary and check_url_existence(taiwan_lines, channel_address):  #港澳台（台湾）
                taiwan_lines.append(process_name_string(line.strip()))
            elif channel_name in international_dictionary and check_url_existence(international_lines, channel_address):  #国际台
                international_lines.append(process_name_string(line.strip()))
            elif channel_name in documentary_dictionary and check_url_existence(documentary_lines, channel_address):  #纪录片
                documentary_lines.append(process_name_string(line.strip()))
            elif channel_name in cartoon_dictionary and check_url_existence(cartoon_lines, channel_address):  #动画片
                cartoon_lines.append(process_name_string(line.strip()))
            elif channel_name in traditional_opera_dictionary and check_url_existence(traditional_opera_lines, channel_address):  #戏曲
                traditional_opera_lines.append(process_name_string(line.strip()))
            elif channel_name in commentary_dictionary and check_url_existence(commentary_lines, channel_address):  #解说
                commentary_lines.append(process_name_string(line.strip()))
            elif channel_name in spring_festival_gala_dictionary and check_url_existence(spring_festival_gala_lines, channel_address):  #春晚
                spring_festival_gala_lines.append(process_name_string(line.strip()))
            elif channel_name in variety_dictionary and check_url_existence(variety_lines, channel_address):  #综艺频道
                variety_lines.append(process_name_string(line.strip()))
            elif channel_name in music_dictionary and check_url_existence(music_lines, channel_address):  #音乐频道
            ...
            elif channel_name in zhejiang_dictionary and check_url_existence(zhejiang_lines, channel_address):  #地方台-浙江频道
                zhejiang_lines.append(process_name_string(line.strip()))
            elif channel_name in jiangsu_dictionary and check_url_existence(jiangsu_lines, channel_address):  #地方台-江苏频道
                jiangsu_lines.append(process_name_string(line.strip()))
            elif channel_name in guangdong_dictionary and check_url_existence(guangdong_lines, channel_address):  #地方台-广东频道
                guangdong_lines.append(process_name_string(line.strip()))
            elif channel_name in hunan_dictionary and check_url_existence(hunan_lines, channel_address):  #地方台-湖南频道
                hunan_lines.append(process_name_string(line.strip()))
            elif channel_name in hubei_dictionary and check_url_existence(hubei_lines, channel_address):  #地方台-湖北频道
                hubei_lines.append(process_name_string(line.strip()))
            elif channel_name in anhui_dictionary and check_url_existence(anhui_lines, channel_address):  #地方台-安徽频道
            ...
            elif channel_name in neimenggu_dictionary and check_url_existence(neimenggu_lines, channel_address):  #地方台-内蒙频道
                neimenggu_lines.append(process_name_string(line.strip()))
            elif channel_name in liaoning_dictionary and check_url_existence(liaoning_lines, channel_address):  #地方台-辽宁频道
                liaoning_lines.append(process_name_string(line.strip()))
            elif channel_name in shaanxi_dictionary and check_url_existence(shaanxi_lines, channel_address):  #地方台-陕西频道
            ...
            elif channel_name in beijing_dictionary and check_url_existence(beijing_lines, channel_address):  #地方台-北京频道
                beijing_lines.append(process_name_string(line.strip()))
            elif channel_name in chongqing_dictionary and check_url_existence(chongqing_lines, channel_address):  #地方台-重庆频道
                chongqing_lines.append(process_name_string(line.strip()))
            elif channel_name in fujian_dictionary and check_url_existence(fujian_lines, channel_address):  #地方台-福建频道
                fujian_lines.append(process_name_string(line.strip()))
            elif channel_name in gansu_dictionary and check_url_existence(gansu_lines, channel_address):  #地方台-甘肃频道
                gansu_lines.append(process_name_string(line.strip()))
            elif channel_name in guangxi_dictionary and check_url_existence(guangxi_lines, channel_address):  #地方台-广西频道
                guangxi_lines.append(process_name_string(line.strip()))
            elif channel_name in guizhou_dictionary and check_url_existence(guizhou_lines, channel_address):  #地方台-贵州频道
                guizhou_lines.append(process_name_string(line.strip()))
            elif channel_name in hebei_dictionary and check_url_existence(hebei_lines, channel_address):  #地方台-河北频道
                hebei_lines.append(process_name_string(line.strip()))
            elif channel_name in henan_dictionary and check_url_existence(henan_lines, channel_address):  #地方台-河南频道
                henan_lines.append(process_name_string(line.strip()))
            elif channel_name in heilongjiang_dictionary and check_url_existence(heilongjiang_lines, channel_address):  #地方台-黑龙江频道
                heilongjiang_lines.append(process_name_string(line.strip()))
            elif channel_name in jilin_dictionary and check_url_existence(jilin_lines, channel_address):  #地方台-吉林频道
                jilin_lines.append(process_name_string(line.strip()))
            elif channel_name in jiangxi_dictionary and check_url_existence(jiangxi_lines, channel_address):  #地方台-江西频道
                jiangxi_lines.append(process_name_string(line.strip()))
            elif channel_name in ningxia_dictionary and check_url_existence(ningxia_lines, channel_address):  #地方台-宁夏频道
                ningxia_lines.append(process_name_string(line.strip()))
            elif channel_name in qinghai_dictionary and check_url_existence(qinghai_lines, channel_address):  #地方台-青海频道
                qinghai_lines.append(process_name_string(line.strip()))
            elif channel_name in sichuan_dictionary and check_url_existence(sichuan_lines, channel_address):  #地方台-四川频道
                sichuan_lines.append(process_name_string(line.strip()))
            elif channel_name in tianjin_dictionary and check_url_existence(tianjin_lines, channel_address):  #地方台-天津频道
                tianjin_lines.append(process_name_string(line.strip()))
            elif channel_name in xinjiang_dictionary and check_url_existence(xinjiang_lines, channel_address):  #地方台-新疆频道
                xinjiang_lines.append(process_name_string(line.strip()))
            elif channel_name in camera_dictionary and check_url_existence(camera_lines, channel_address):  #直播中国
                camera_lines.append(process_name_string(line.strip()))
            elif channel_name in digital_dictionary and check_url_existence(digital_lines, channel_address):  #数字频道（对应auto_4k）
                digital_lines.append(process_name_string(line.strip()))
            # 其他v0.00版本没有的分类暂时放入other_lines
            else:
...
print(f"   省级地方台: 34个分类已加载")
print(f"   港澳台: 3个分类已加载")
print(f"   其他分类: 25个分类已加载")
...
print(f"✅ 体育赛事网页已生成: output/tiyu.html")
...
print("📄 生成播放列表文件")
# 构建完整版播放列表（使用v0.01的结构，但保持v0.00的分类内容）
all_lines_full =  ["🌐央视频道,#genre#"] + sorted(yangshi_lines, key=lambda x: (custom_sort(x), extract_number(x))) + ['\n'] + \
        ...
        ["🎭综艺频道,#genre#"] + sort_data(variety_dictionary, set(correct_name_data(corrections_name, variety_lines))) + ['\n'] + \
        ...
        ["🎤解说频道,#genre#"] + sort_data(commentary_dictionary, set(commentary_lines)) + ['\n'] + \
        ["🎵音乐频道,#genre#"] + sort_data(music_dictionary, set(music_lines)) + ['\n'] + \
        ...
        ["🎮游戏频道,#genre#"] + sort_data(game_dictionary, set(game_lines)) + ['\n'] + \
        ...
        ["🏞️景区直播,#genre#"] + sort_data(camera_dictionary, set(correct_name_data(corrections_name,camera_lines))) + ['\n'] + \
...
# 构建精简版播放列表
all_lines_lite =  ["🌐央视频道,#genre#"] + sorted(yangshi_lines, key=lambda x: (custom_sort(x), extract_number(x))) + ['\n'] + \
        ["📡卫视频道,#genre#"] + sort_data(weishi_dictionary,correct_name_data(corrections_name,weishi_lines)) + ['\n'] + \
        ["🕒更新时间,#genre#"] + [version] + [about] + [MTV1] + [MTV2] + [MTV3] + [MTV4] + [MTV5] + read_txt_to_array('assets/livesource/手工区/about.txt') + ['\n']

# 构建定制版播放列表
all_lines_custom = ["🌐央视频道,#genre#"] + sorted(yangshi_lines, key=lambda x: (custom_sort(x), extract_number(x))) + ['\n'] + \
        ["📡卫视频道,#genre#"] + sort_data(weishi_dictionary, correct_name_data(corrections_name, weishi_lines)) + ['\n'] + \
        ["💓专享央视,#genre#"] + read_txt_to_array('assets/livesource/手工区/优质央视.txt') + ['\n'] + \
        ["💓专享卫视,#genre#"] + read_txt_to_array('assets/livesource/手工区/优质卫视.txt') + ['\n'] + \
        ["⚽️SPORTS,#genre#"] + read_txt_to_array('assets/livesource/手工区/sports.txt') + ['\n'] + \
