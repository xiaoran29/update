基于您的要求，我将基于v1.01版本，采用方案一（基于字典的规则引擎）进行优化，推出 v2.1版本。
...
def setup_channel_rules():
    ...
    rules.append({
        ...
        'target': yangshi_lines,
        'description': 'CCTV开头的频道'
    })
    ...
    rules.append({
        ...
        'target': weishi_lines,
        'description': '卫视字典精确匹配'
    })
    
    # 3. 体育赛事（特征匹配，先于普通体育频道）
    rules.append({
        'name': '体育赛事',
    ...
    for rule_name, dictionary, target_list in province_rules:
        rules.append({
            'name': rule_name,
            'condition': lambda cn, dict=dictionary: cn in dict,
            'target': target_list,
            'description': f'{rule_name}字典匹配'
        })
    ...
    for rule_name, dictionary, target_list in other_category_rules:
        rules.append({
            'name': rule_name,
            'condition': lambda cn, dict=dictionary: cn in dict,
            'target': target_list,
...
def process_channel_line_v2_1(line):
    """v2.1: 基于规则引擎的频道分类函数"""
    global channel_rules
    
    # 初始化规则引擎（第一次运行时）
    if channel_rules is None:
        channel_rules = setup_channel_rules()
        print(f"频道规则引擎已初始化，共 {len(channel_rules)} 条规则")
    ...
    if "#genre#" not in line and "#EXTINF:" not in line and "," in line and "://" in line:
        ...
        for rule in channel_rules:
            try:
                # 检查规则条件
                if rule['condition'](channel_name):
                    # 检查URL是否已存在
                    if check_url_existence(rule['target'], channel_address):
                        # 处理频道名称格式并添加
                        rule['target'].append(process_name_string(processed_line))
                        return  # 匹配成功，立即返回
            except Exception as e:
            ...
            for line in lines:
                ...
                if "#genre#" not in line and "," in line and "://" in line and "tvbus://" not in line and "/udp/" not in line:
                    ...
                    if "#" not in channel_address:
                        # 普通源，直接处理
                        process_channel_line_v2_1(line)
                    else: 
                        # 加速源，按#号分割为多个URL分别处理
                        url_list = channel_address.split('#')
                        for channel_url in url_list:
                            newline = f'{channel_name},{channel_url}'
                            process_channel_line_v2_1(newline)
...
print(f"直播源聚合处理工具 v2.1")
print(f"基于规则引擎的频道分类系统")
print(f"规则引擎初始化中...")
...
for whitelist_line in whitelist_auto_lines:
    if "#genre#" not in whitelist_line and "," in whitelist_line and "://" in whitelist_line:
        ...
        if response_time < 2000:
            process_channel_line_v2_1(",".join(whitelist_parts[1:]))
...
# 简单统计
print(f"\n=== v2.1 规则引擎统计 ===")
print(f"规则总数: {len(channel_rules) if channel_rules else 0}")
...
1. 规则引擎分类系统：替代了原来的硬编码if-elif结构
2. 可配置的规则优先级：规则按优先级顺序执行
3. 易于扩展：添加新分类只需在 setup_channel_rules() 中添加新规则
4. 错误处理：每条规则都有try-catch保护

✅ 规则特点：

1. 优先级清晰：央视 > 卫视 > 体育赛事 > 字典匹配 > 其他
2. 条件精确：每条规则都有详细的匹配条件
3. 描述清晰：每条规则都有描述信息，便于调试
4. 统一处理：URL存在性检查、名称处理都在统一流程中
...
这个版本是测试版，建议先用小量数据测试，确保规则引擎工作正常后再进行大规模数据处理。