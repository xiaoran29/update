我来帮您修改代码，添加这些重要的打印信息。以下是完整的修改后代码：
...
# 记录执行开始时间
utc_time_start = datetime.now(timezone.utc)
timestart = utc_time_start + timedelta(hours=8)
...
print(f"✅ 已添加 {whitelist_count} 个高质量白名单源")
...
zhejiang_lines = zhejiang_lines + zhejiang_manual
print(f"   浙江频道: 添加 {len(zhejiang_manual)} 个手工源")

guangdong_manual = read_txt_to_array('assets/livesource/手工区/广东频道.txt')
guangdong_lines = guangdong_lines + guangdong_manual
print(f"   广东频道: 添加 {len(guangdong_manual)} 个手工源")

hubei_manual = read_txt_to_array('assets/livesource/手工区/湖北频道.txt')
hubei_lines = hubei_lines + hubei_manual
print(f"   湖北频道: 添加 {len(hubei_manual)} 个手工源")

shanghai_manual = read_txt_to_array('assets/livesource/手工区/上海频道.txt')
shanghai_lines = shanghai_lines + shanghai_manual
print(f"   上海频道: 添加 {len(shanghai_manual)} 个手工源")

jiangsu_manual = read_txt_to_array('assets/livesource/手工区/江苏频道.txt')
jiangsu_lines = jiangsu_lines + jiangsu_manual
print(f"   江苏频道: 添加 {len(jiangsu_manual)} 个手工源")
...
# 计算执行时间
utc_time_end = datetime.now(timezone.utc)
timeend = utc_time_end + timedelta(hours=8)
...
print(f"🔄 去重率: {(1 - len(processed_urls) / (len(processed_urls) + combined_blacklist_count)) * 100:.1f}%" if (len(processed_urls) + combined_blacklist_count) > 0 else "去重率: N/A")

print(f"📊 分类统计:")
print(f"   央视频道: {len(yangshi_lines)} 个")
print(f"   卫视频道: {len(weishi_lines)} 个")
print(f"   体育赛事: {len(filtered_tyss_lines)} 个")
print(f"   其他频道: {len(other_lines)} 个")
...
1. URL处理进度：每个URL开始处理时都有提示
2. 黑名单过滤：显示哪些频道被黑名单过滤
3. URL去重：显示重复URL的去重
4. 处理统计：每个URL处理多少频道
5. 白名单统计：显示添加了多少高质量白名单源
6. 分类统计：详细的频道分类数量
7. 文件生成确认：每个文件生成都有明确提示
8. 去重率计算：显示整体的去重效果
9. 手工区统计：显示手工添加的源数量

现在脚本运行时会有丰富的状态提示，便于监控和调试！
打印展示
创建输出目录: output
开始时间: 20241228 15:30:25
开始处理 5 个数据源

📡 开始处理URL: http://example1.com/source1.m3u
  行数: 150
🚫 黑名单过滤: 测试频道1
🚫 黑名单过滤: 测试频道2
🔄 URL去重: CCTV1
...
📡 开始处理URL: http://example2.com/live.txt  
  行数: 89
🚫 黑名单过滤: 测试频道3
  成功处理: 86 个频道

📡 开始处理URL: http://example3.com/iptv.m3u8
  行数: 200
🚫 黑名单过滤: 测试频道4
🔄 URL去重: CCTV5
  成功处理: 195 个频道

📡 开始处理URL: http://example4.com/tv.txt
  行数: 75
  成功处理: 73 个频道

📡 开始处理URL: http://example5.com/list.m3u
  行数: 120
🚫 黑名单过滤: 测试频道5
  成功处理: 117 个频道
...
🏆 体育赛事处理完成：原始 150 条，过滤后 125 条
✅ 网页已生成：output/tiyu.html
✅ 体育赛事文本已保存到文件: tiyu.txt
...
time: 20241228 15:31:10

✅ M3U文件已生成 'output/full.m3u'
✅ 完整版M3U已生成: output/full.m3u

✅ M3U文件已生成 'output/lite.m3u'
✅ 精简版M3U已生成: output/lite.m3u

✅ M3U文件已生成 'output/custom.m3u'
✅ 定制版M3U已生成: output/custom.m3u

开始时间: 20241228 15:30:25
结束时间: 20241228 15:31:10
执行时间: 0 分 45 秒
...
问得好！手工维护的频道同样要经过黑名单检查！

手工频道的处理流程：

1. 手工频道的加载（第357-366行附近）
...
2. 关键点：手工频道是直接添加到分类列表，而不是调用process_channel_line()
...
```
普通URL → process_channel_line() → 黑名单检查 → 通过 → 添加到分类列表
               ↓
手工频道 → 直接添加到分类列表 → ❌ 跳过了黑名单检查！
```
...
· 黑名单：http://bad.com/tv.m3u8
· 手工文件湖北频道.txt中有一行：湖北测试,http://bad.com/tv.m3u8

结果：这个频道会直接进入播放列表，因为跳过了黑名单检查！

解决方案：

需要修改手工频道的处理逻辑：

修改方案1：使用process_channel_line处理
...
# 读取手工频道文件
manual_files = {
    '浙江频道': 'assets/livesource/手工区/浙江频道.txt',
    '广东频道': 'assets/livesource/手工区/广东频道.txt',
    '湖北频道': 'assets/livesource/手工区/湖北频道.txt',
    '上海频道': 'assets/livesource/手工区/上海频道.txt',
    '江苏频道': 'assets/livesource/手工区/江苏频道.txt'
}

for name, file_path in manual_files.items():
    manual_lines = read_txt_to_array(file_path)
    print(f"   处理{name}: {len(manual_lines)} 行")
    
    for line in manual_lines:
        process_channel_line(line)  # ✅ 这样就会检查黑名单了
...
```python
def filter_manual_channels(manual_lines):
    """过滤手工频道中的黑名单"""
    filtered_lines = []
    for line in manual_lines:
        if "#genre#" not in line and "," in line and "://" in line:
            ...
            if len(parts) >= 2:
                ...
                # 黑名单检查
                if channel_address in combined_blacklist:
                    print(f"🚫 手工区黑名单过滤: {parts[0].strip()}")
                    continue  # 跳过这个频道
...
# 使用时
zhejiang_manual = read_txt_to_array('assets/livesource/手工区/浙江频道.txt')
zhejiang_manual_filtered = filter_manual_channels(zhejiang_manual)
zhejiang_lines = zhejiang_lines + zhejiang_manual_filtered
```

建议：

必须修复这个安全问题！ 否则手工频道会成为黑名单的漏洞。

推荐使用方案1，因为：

1. 一致性：所有频道都通过process_channel_line()处理
2. 完整检查：不仅检查黑名单，还有去重、名称清理等
3. 维护简单：逻辑统一

当前代码的风险：

1. 安全漏洞：黑名单可能被手工频道绕过
2. 数据污染：手工文件中可能包含失效或违规链接
3. 质量下降：影响整体播放列表质量

结论：您发现了脚本的一个重要安全问题！手工频道确实绕过了黑名单检查，需要立即修复。