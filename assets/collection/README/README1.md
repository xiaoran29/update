...
https://img.shields.io/badge/版本-v1.01-green.svg
https://img.shields.io/badge/许可证-MIT-yellow.svg
https://img.shields.io/badge/更新-2025年5月-brightgreen.svg

一个功能强大的自动化IPTV直播源收集、分类、过滤和转换工具，能够从多个来源聚合直播源并生成标准化播放列表。

✨ 特性亮点

· 🔄 多源聚合 - 支持从多个在线源自动收集直播源
· 🏷️ 智能分类 - 自动识别并分类到60+个频道类别
· 🚫 智能过滤 - 基于黑名单/白名单系统过滤无效源
· 📊 质量优选 - 基于响应时间筛选高质量直播源
· 📁 多格式输出 - 支持TXT、M3U、HTML等多种格式
· 🔄 自动更新 - 支持动态日期变量，每日自动更新
· 🛡️ 健壮性 - 完善的错误处理和重试机制
...
编辑 assets/livesource/urls-daily.txt 文件，添加或修改直播源URL：

```
# 格式：每行一个URL
http://example.com/source1.txt
http://example.com/source2.m3u
https://example.com/{MMdd}/live.txt  # 支持日期变量
```
...
每个分类对应一个字典文件，位于 assets/livesource/主频道/ 和 assets/livesource/地方台/ 目录。
...
编辑 assets/livesource/corrections_name.txt：

```
正确名称,错误名称1,错误名称2,...
CCTV1,CCTV-1,CCTV 1,中央1台
```
...
```
🏛️ 北京频道   🏙️ 上海频道   🐯 广东频道   🎐 江苏频道
🧵 浙江频道   ⛰️ 山东频道   🐼 四川频道   🐘 河南频道
🌶️ 湖南频道   🏞️ 重庆频道   🚢 天津频道   🏯 湖北频道
...
包含所有60+个分类的完整播放列表，适合作为主播放列表使用。

精简版 (output/lite.*)

只包含央视频道和卫视频道，适合网络环境较差时使用。
...
未分类源 (output/others.txt)

未能自动分类的直播源，可用于手动检查和补充。

🔧 高级功能

1. 频道Logo支持

在 assets/livesource/logo.txt 中添加频道Logo：
...
在 assets/livesource/手工区/ 目录添加手工整理的频道数据，程序会自动合并。
...
1. 在process_channel_line函数中添加分类逻辑：

```python
elif channel_name in new_category_dictionary:
    new_category_lines.append(line)
```

修改过滤规则

编辑 removal_list 变量修改频道名称清理规则：

```python
removal_list = ["_电信", "高清", "频道", "HD", "标清", ...]
```
...
- 🏛️ 央视频道 (CCTV系列)：CCTV1-CCTV17, 4K/8K超高清频道
- 📡 卫视频道 (各省卫视)：湖南卫视、浙江卫视、江苏卫视等

🗺️ 地方台分类（27个省份）

- 🏙️ 直辖市：北京频道、上海频道、天津频道、重庆频道
- 🏞️ 省份频道：广东、江苏、浙江、山东、河南、四川等
...
- 🏛️ CCTV标准化：统一CCTV频道命名格式，智能识别频道编号
- 📡 卫视清理：移除卫视名称中的冗余信息和特殊标记
- 🖥️ 4K/8K识别：智能识别和处理超高清频道标识
...
```python
# 🌐 网络请求配置
REQUEST_TIMEOUT = 10        # ⏱️ 请求超时时间(秒)，平衡成功率和效率
...
这个脚本是一个功能完整、设计专业的直播源处理系统，适合需要高质量电视直播频道管理的各种应用场景。通过合理的配置和维护，可以提供稳定可靠的直播源服务。.