...
# 地方台统计
print(f"   地方台频道:")
print(f"     北京: {len(beijing_lines)} 上海: {len(shanghai_lines)} 广东: {len(guangdong_lines)}")
print(f"     江苏: {len(jiangsu_lines)} 浙江: {len(zhejiang_lines)} 山东: {len(shandong_lines)}")
print(f"     四川: {len(sichuan_lines)} 河南: {len(henan_lines)} 湖南: {len(hunan_lines)}")
print(f"     重庆: {len(chongqing_lines)} 天津: {len(tianjin_lines)} 湖北: {len(hubei_lines)}")
print(f"     安徽: {len(anhui_lines)} 福建: {len(fujian_lines)}辽宁: {len(liaoning_lines)}")
print(f"     陕西: {len(shaanxi_lines)} 河北: {len(hebei_lines)} 江西: {len(jiangxi_lines)}")
print(f"     广西: {len(guangxi_lines)} 云南: {len(yunnan_lines)} 山西: {len(shanxi_lines)}")
print(f"     黑龙江: {len(heilongjiang_lines)} 吉林: {len(jilin_lines)} 贵州: {len(guizhou_lines)}")
print(f"     甘肃: {len(gansu_lines)} 内蒙古: {len(neimenggu_lines)} 新疆: {len(xinjiang_lines)}")
print(f"     海南: {len(hainan_lines)} 宁夏: {len(ningxia_lines)} 青海: {len(qinghai_lines)}")
print(f"     西藏: {len(xizang_lines)}")

# 港澳台统计
print(f"   港澳台频道:")
print(f"     香港: {len(hongkong_lines)} 澳门: {len(macau_lines)} 台湾: {len(taiwan_lines)}")

# 定制台统计
print(f"   定制台频道:")
print(f"     数字: {len(digital_lines)} 电影: {len(movie_lines)} 电视剧: {len(tv_drama_lines)}")
print(f"     纪录片: {len(documentary_lines)} 动画片: {len(cartoon_lines)} 收音机: {len(radio_lines)}")
print(f"     综艺: {len(variety_lines)} 虎牙: {len(huya_lines)} 斗鱼: {len(douyu_lines)}")
print(f"     解说: {len(commentary_lines)} 音乐: {len(music_lines)} 美食: {len(food_lines)}")
print(f"     旅游: {len(travel_lines)} 健康: {len(health_lines)} 财经: {len(finance_lines)}")
print(f"     购物: {len(shopping_lines)} 游戏: {len(game_lines)} 新闻: {len(news_lines)}")
print(f"     中国综合: {len(china_lines)} 国际: {len(international_lines)} 体育: {len(sports_lines)}")
print(f"     体育赛事: {len(filtered_tyss_lines)} 咪咕赛事: {len(mgss_lines)}")
print(f"     戏曲: {len(traditional_opera_lines)} 春晚: {len(spring_festival_gala_lines)}")
print(f"     景区直播: {len(camera_lines)} 收藏频道: {len(favorite_lines)}")
print(f"     其他未分类: {len(other_lines)}")

# 计算频道总数
total_channels = (
    len(yangshi_lines) + len(weishi_lines) +
    len(beijing_lines) + len(shanghai_lines) + len(guangdong_lines) +
    len(jiangsu_lines) + len(zhejiang_lines) + len(shandong_lines) +
    len(sichuan_lines) + len(henan_lines) + len(hunan_lines) +
    len(chongqing_lines) + len(tianjin_lines) + len(hubei_lines) +
    len(anhui_lines) + len(fujian_lines) + len(liaoning_lines) +
    len(shaanxi_lines) + len(hebei_lines) + len(jiangxi_lines) +
    len(guangxi_lines) + len(yunnan_lines) + len(shanxi_lines) +
    len(heilongjiang_lines) + len(jilin_lines) + len(guizhou_lines) +
    len(gansu_lines) + len(neimenggu_lines) + len(xinjiang_lines) +
    len(hainan_lines) + len(ningxia_lines) + len(qinghai_lines) +
    len(xizang_lines) + len(hongkong_lines) + len(macau_lines) +
    len(taiwan_lines) + len(digital_lines) + len(movie_lines) +
    len(tv_drama_lines) + len(documentary_lines) + len(cartoon_lines) +
    len(radio_lines) + len(variety_lines) + len(huya_lines) +
    len(douyu_lines) + len(commentary_lines) + len(music_lines) +
    len(food_lines) + len(travel_lines) + len(health_lines) +
    len(finance_lines) + len(shopping_lines) + len(game_lines) +
    len(news_lines) + len(china_lines) + len(international_lines) +
    len(sports_lines) + len(filtered_tyss_lines) + len(mgss_lines) +
    len(traditional_opera_lines) + len(spring_festival_gala_lines) +
    len(camera_lines) + len(favorite_lines) + len(other_lines)
    )
